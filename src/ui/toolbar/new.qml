...
Item {
    id:             _root
    width:          (gpsValuesColumn.x + gpsValuesColumn.width) * 1.1 // icon width
    anchors.top:    parent.top
    anchors.bottom: parent.bottom
    visible: (_activeVehicle.roll.value.toFixed(2)>35 ^ _activeVehicle.roll.value.toFixed(2)<-35 ^ _activeVehicle.pitch.value.toFixed(2)<-35 ^ _activeVehicle.pitch.value.toFixed(2)>35)
    ...
    Component {
        ...
        Rectangle {
            width:  gpsCol.width   + ScreenTools.defaultFontPixelWidth  * 3   // gps status width
            ...
            Column {
                ...
                width:              Math.max(gpsInfo.width) //(gpsGrid.width, gpsLabel.width)
      ...
      /*          GridLayout {
                 ...
                 //   QGCLabel { text: _activeVehicle ? _activeVehicle.gps.courseOverGround.valueString : qsTr("--.--", "No data to display") }
                     QGCLabel { text: _activeVehicle ? _activeVehicle.mavlinkReceivedCount : qsTr("--.--", "No data to display") }
    ...
    QGCColoredImage {
      ...
      //  opacity:            (_activeVehicle && _activeVehicle.gps.count.value >= 0) ? 1 : 0.5
        color:              qgcPal.surveyPolygonTerrainCollision//qgcPal.warningText
    }
